...
import ModalPreview, {
  ModalDismissalPreview,
  ModalFocusPreview,
  ModalShiftPreview,
  PortalLayerPreview,
} from "@/components/blog/previews/modals";
...
## Dismissal is a contract

Opening the dialog is only half of the interaction. Every way out should call the same `onClose` function: the close button, the backdrop, and the `Escape` key.

The backdrop owns the outside click. The surface stops that click from bubbling, so pressing a field or a button inside the dialog does not dismiss it by accident.
...
The important part is not the backdrop color. It is the boundary between the page and the dialog surface. Outside input closes the temporary context. Inside input stays inside it.
...
That small effect changes the feel of the whole layer. The background no longer drifts while someone is trying to read or act inside the dialog.

Focus needs the same care. When the modal opens, move focus into it. While it is open, keep `Tab` inside the dialog. When it closes, send focus back to the element that opened it. The browser will not do all of that work for you.
...
The `Tab` loop keeps keyboard focus inside the temporary context. The cleanup restores the previous focus target, so closing the modal returns the reader to the action that opened it.

`Escape` is still part of the behavior, but it sits inside a bigger contract: the modal is a temporary context. While that context is active, keyboard movement should stay inside it and page scroll should wait.
...
I prefer applying the compensation to the root element because it avoids fighting page-level body transitions. The important part is restoring both values when the modal closes.
...
## One lifecycle, one boundary

The modal is easiest to reason about when every behavior derives from the same `open` state and exits through the same `onClose` callback.
...
function ShareModal({ open, onClose, children }: ModalProps) {
  const dialogRef = useRef<HTMLDivElement>(null);
...
That boundary gives the component one predictable lifecycle: render through the portal, lock the page, move focus into the dialog, handle dismissal, then restore the page and the previous focus target during cleanup.

The surface can change from a share sheet to a form or a confirmation dialog without changing those rules. The content belongs inside the boundary. The boundary owns the behavior around it.
