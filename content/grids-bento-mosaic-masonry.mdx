...
## Four decisions behind every layout
...
## When layout becomes motion

CSS Grid still decides the final geometry. Framer Motion only interpolates the boxes between the old arrangement and the new one. That separation keeps the layout rules readable and gives the transition one source of truth.
...
<motion.div
  ...
  style={{ gridTemplateColumns: columns[count] }}
>
  {items.map((item) => (
...
The `layout` prop measures each element before and after the state change, then animates the difference. Stable keys let Motion match the same card across both layouts. `AnimatePresence` handles cards entering or leaving; it does not replace the grid’s placement rules.

Keep CSS responsible for tracks, gaps, spans, and flow. Use Motion to make those geometry changes legible.
...
Grid can change where an item appears without changing the order in the markup. That is useful for composition, but it becomes a problem when the visual order is also the user’s task order.
...
Keep meaningful content in the order it should be read, then use tracks and areas to shape the surface around it. Treat `dense` placement and manual `order` changes as presentation tools for galleries and dashboards, not as a substitute for a sensible DOM sequence.

Keyboard navigation and assistive technology follow the document structure. A layout that looks tidy but makes the next action difficult to predict is still the wrong layout.
