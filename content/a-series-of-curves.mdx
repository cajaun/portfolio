...
The chart uses a value range from zero to one hundred. `getPointY` reverses that range because SVG increases `y` as it moves down the screen. `buildChartPoints` distributes each sample across the current graph width. The path, pointer lookup, clip rectangles and cursor all use these same units.
...
The important choice sits outside the formula. The stroke, the area fill, the live clip and the future clip all consume the same path. That gives the chart one geometry source. A cursor evaluated against the same Bézier segments lands on the visible curve instead of stopping at the nearest raw sample.
...
The fill path closes against the chart floor, so it inherits every change in the line. The width motion value drives the endpoint marker and the clip rectangles. When the selected range changes, the visible line, the area and the graph boundary travel as one state change.
...
The pointer surface needs to cover the graph without changing the geometry. An invisible SVG rectangle gives the chart a stable hit area, while pointer capture keeps the drag alive when the cursor crosses a curve or moves beyond a narrow line.
...
The returned point carries both screen position and value. The chart uses `x` to place the guide and split the path and it uses `y` to place the cursor on the stroke. A single lookup keeps those visual details aligned during a scrub.
...
Scrubbing changes the meaning of the line. The portion before the cursor stays active, while the portion after the cursor becomes a preview of what follows. Both portions must come from the same animated path. Two clip paths divide the line at the cursor without creating a second geometry calculation.
...
The range control changes data, so it belongs beside the chart without entering the chart’s geometry. The preview card owns the footer boundary. The stage owns the edge-to-edge chart surface. The footer keeps the site’s animated tab primitive and passes a controlled value back to the chart.
...
The `setCursor(null)` call gives a range change a clean interaction state. The plot then morphs from the old series into the new one and the animated tabs keep the selected range visible in the footer.
...
The chart surface has a narrow boundary. It accepts values, a range, a cursor, a series and a fill flag. The hooks handle motion and pointer state. The SVG owns geometry. The preview card owns composition. Each boundary keeps the next change local.
