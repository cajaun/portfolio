...
## Preparing feed cards

You can render a cheap image card below the fold, then expand it into the real media grid before the reader reaches it. The observer watches each card inside the feed root and marks it ready once it enters the prep zone.

<FeedPreparationPreview />

That gives image decoding, media setup, and small fetches a head start. You do not need scroll math for each card. You only need to know when the card gets close enough to prepare.

```tsx
const [preparedPosts, setPreparedPosts] = useState(
  () => new Set(["design-camera"]),
);
...
const observer = new IntersectionObserver(
  (entries) => {
    entries.forEach((entry) => {
      ...
      if (id) {
        setPreparedPosts((posts) => new Set(posts).add(id));
      }
    });
  },
  {
...
feedCards.map((card) => (
  <ImageCardSkeleton
    key={card.id}
    count={preparedPosts.has(card.id) ? card.imageCount : 1}
    title={card.title}
  />
));
```

The bottom `rootMargin` expands the feed root downward. The next card can switch from the one-block placeholder to its full grid before it reaches the middle of the viewport.
