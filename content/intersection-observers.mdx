...
The same idea works inside horizontal containers. A carousel can observe each card against its own scroll root and start work before the card is fully centered.

<CarouselPrefetchPreview />

That is useful for image decoding, media preparation, or lightweight data prefetching. You do not need to watch every horizontal scroll event. You only need to know when a card is close enough to be worth preparing.
...
const observer = new IntersectionObserver(
  (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        preloadCard((entry.target as HTMLElement).dataset.cardId!);
      }
    });
  },
  {
    root: carouselScroller,
    threshold: 0.25,
...
The horizontal `rootMargin` expands the carousel root on both sides. That means the next card can become ready before the user lands on it, without coupling the behavior to hand-written scroll math.
