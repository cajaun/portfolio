...
export function ScrollIsland({
  ...
  return (
    <div
      className="relative w-full max-w-screen-sm mx-auto pb-40"
      ref={setupTitles}
    >
        ...
        >
          {sections.map((item, index) => (
            <Fragment key={item.id}>
              <h2
                id={item.id}
                className="pt-8 font-bold animate-slide-down-fade"
          ...
          <ol className="px-4 pt-4" style={{ paddingBlockEnd: 44 }}>
            {sections.map((item) => (
              <motion.li
                key={item.id}
                className={`
                  list-inside list-decimal transition-[filter] font-medium  ${
